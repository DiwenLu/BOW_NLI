...
class NeuralNetworkPytorch(nn.Module):
    ...
    def forward(self, data_hypo, length_hypo, data_prem, length_prem, interaction_type):
        ...
        @param length: an int tensor of size (batch_size), which represents the non-trivial (excludes padding) length of each sentences in the data.
        @param data_prem: matrix of size (batch_size, max_sentence_length).
        @param length_hypo: an int tensor of size (batch_size), which represents the non-trivial (excludes padding) length of each sentences in the data_prem.
        ...
        m_hypo = (x_hypo == 1)
        m_prem= (x_prem == 1)
        m_hypo = m_hypo.unsqueeze(2).repeat(1, 1, 300).type(torch.FloatTensor)#.to(device)
        m_prem = m_prem.unsqueeze(2).repeat(1, 1, 300).type(torch.FloatTensor)#.to(device)
        m_hypo=torch.sum(m_hypo,dim=1)
...
if __name__ == '__main__':
    ...
    # Vocabulary
    # buid vocabulary index token according to max_vocab_size
    ...
    for epoch in range(num_epochs):
        ...
        # define state that need to be saved
        state = {'epoch': epoch + 1,
        ...
        torch.save(state, 'checkpoint{}_{}.pth.tar'.format(epoch + 1, 'unknowns_learned'))
